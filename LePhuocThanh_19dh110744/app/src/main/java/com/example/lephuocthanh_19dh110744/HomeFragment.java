...
public class HomeFragment extends Fragment implements HomeFurnitureAdapter.Listener {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        rvFurniture =view.findViewById(R.id.rvFurniture);
        arrayList.clear();
        if(Utilities.getInstanceFromCate().size()==0){
            arrayList =App.init(getContext()); //toan bo furnitre
        }
        else{
            arrayList=Utilities.getInstanceFromCate();
        }

        homeFurnitureAdapter=new HomeFurnitureAdapter(arrayList,this);
    ...
    public  ArrayList<Furniture> init(){
    ...
    public void Onclick(Furniture furniture) {
        if(Utilities.getInstance().indexOf(furniture)<0) {
